...
class ActivityLogsService
{
    ...
    public function getFilteredQuery(array $filters = []): Builder
    {
        ...
        if (! empty($filters['log_name'])) {
            $query->where('sys_activity_log.log_name', $filters['log_name']);
        }

        if (! empty($filters['event'])) {
            $query->where('sys_activity_log.event', $filters['event']);
        }
        ...
        if (! empty($filters['subject_type'])) {
            $query->where('sys_activity_log.subject_type', $filters['subject_type']);
        }

        if (! empty($filters['causer_id'])) {
            $query->where('sys_activity_log.causer_id', $filters['causer_id']);
        }
     ...
     */
    public function buildFiltersFromRequest($request): array
    {
        $filters = [];
        ...
        if ($request->has('search') && $request->search['value'] != '') {
            $filters['search_value'] = $request->search['value'];
        }
