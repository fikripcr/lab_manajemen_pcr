...
class PresensiService
{
     ...
     */
    public function uploadPhoto(User $user, ?UploadedFile $photo, ?string $faceEncodingInput): array
    {
        ...
        if ($faceEncodingInput) {
            $pegawai->face_encoding = $faceEncodingInput;
        } else {
    ...
    public function getPresensiHistory(array $filters): array
    {
        ...
        $filtered = array_filter($data, function ($item) use ($filters) {
            ...
            if (isset($filters['month']) && $filters['month'] !== 'all') {
                if ($date->format('m') !== $filters['month']) {
                    return false;
                }
            }
