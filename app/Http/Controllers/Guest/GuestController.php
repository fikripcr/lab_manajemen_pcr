...
class GuestController extends Controller
{
    public function home(){
        ...
        $approvedSoftwareRequests = \App\Models\RequestSoftware::where('status', 'disetujui')
            ->with(['mataKuliahs'])
            ->orderBy('created_at', 'desc')
            ->limit(6)
            ->get();
    ...
    public function requestSoftware(){
        $mataKuliahs = \App\Models\MataKuliah::all(); // For old values display
    ...
    public function getMataKuliah(Request $request)
    {
        ...
        $mataKuliahs = \App\Models\MataKuliah::when($search, function ($query, $search) {
