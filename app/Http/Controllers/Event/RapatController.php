...
class RapatController extends Controller
{
    ...
    public function create()
    {
        ...
        if (request()->has('jenis_rapat')) {
            $rapat->jenis_rapat = request('jenis_rapat');
        }

        // Handle pre-filled agendas from query parameter (comma separated)
        $prefilledAgendas = [];
        if (request()->has('pre_agendas')) {
            $titles = explode(',', request('pre_agendas'));
            foreach ($titles as $title) {
                if ($title = trim($title)) {
                    $prefilledAgendas[] = (object) ['judul_agenda' => $title, 'isi' => ''];
                }
            }
        }

        $entitasType = request('entitas_type');
        ...
        return view('pages.event.rapat.create-edit-ajax', compact(
            'pageTitle', 'users', 'rapat', 'defaultDate', 'defaultStartTime', 'defaultEndTime',
            'prefilledAgendas', 'entitasType', 'entitasId'
        ));
    }
    ...
    public function store(RapatRequest $request)
    {
        ...
        // Link to entitas if provided
        if ($request->filled(['entitas_type', 'entitas_id'])) {
            $this->RapatService->addEntitas($rapat, [
                'model' => $request->entitas_type,
                'model_id' => $request->entitas_id, // Already decrypted/prepared by RapatRequest
                'keterangan' => 'Linked from ' . $request->entitas_type . ' (Automatic)',
            ]);
        }
