...
class PemutuSeeder extends Seeder
{
    public function run()
    {
        ...
        // Attach label manually after creation if needed, or check relationship method
        // $indikator1->labels()->attach([$createdLabels[0]->label_id]);
        // The original code chained ->labels()->attach() on create(), which works if create returns the model.
        // But Indikator::create returns the model, so it should work.
        // However, let's separate it to be safe and clear.
        $indikator1 = Indikator::where('no_indikator', 'V-01')->first();
        if ($indikator1) {
            $indikator1->labels()->attach([$createdLabels[0]->label_id]);
        }
        ...
        $indikator2 = Indikator::where('no_indikator', 'V-02')->first();
        if ($indikator2) {
            $indikator2->orgUnits()->attach([$bpm->orgunit_id, $wadir1->orgunit_id]);
        }
        ...
        $manual = Dokumen::create([
            ...
            'kode'           => 'MM-SPMI-01',
            'jenis'          => 'dll', // Manual Mutu not in enum
            'periode'        => 2025,
