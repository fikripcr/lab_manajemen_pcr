<x-tabler.form-modal
    title="{{ isset($rapat) && $rapat->exists ? 'Edit Data RTM Peningkatan' : 'Buat RTM Peningkatan' }}"
    route="{{ isset($rapat) && $rapat->exists
        ? route('pemutu.peningkatan.rtm.update', [$periode->encrypted_periodespmi_id, $rapat->encrypted_rapat_id])
        : route('pemutu.peningkatan.rtm.store', $periode->encrypted_periodespmi_id) }}"
    ...
    <div class="alert alert-info mt-3 mb-0 p-2">
        ...
        <strong>2 agenda default</strong> akan otomatis dibuat: Rangkuman dan Penggunaan Budget.
    </div>
    @endif
