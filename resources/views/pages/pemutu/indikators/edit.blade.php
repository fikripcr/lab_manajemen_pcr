@if(request()->ajax() || request()->has('ajax'))
    <x-tabler.form-modal
        ...
        route="{{ route('pemutu.indikators.update', $indikator->indikator_id) }}"
        method="PUT"
        size="modal-xl"
        submitText="Update Indikator"
    >
        <div class="mb-3">
            <ul class="nav nav-tabs" data-bs-toggle="tabs">
                ...
                <li class="nav-item" id="modal-nav-hierarchy" style="{{ $indikator->type != 'performa' ? 'display: none;' : '' }}">
                    <a href="#modal-tabs-hierarchy" class="nav-link" data-bs-toggle="tab"><i class="ti ti-hierarchy-2 me-2"></i>Struktur Hirarkis</a>
                </li>
                <li class="nav-item" id="modal-nav-target" style="{{ $indikator->type == 'performa' ? 'display: none;' : '' }}">
                            ...
                            <table class="table table-vcenter card-table table-striped">
                                ...
                                <tbody>
                                    @php
                                        ...
                                        if(!function_exists('renderUnitRowModalEdit')){
                                            function renderUnitRowModalEdit($unit, $level = 0, $assignedMap) {
                                                $padding = $level * 20;
                                                ...
                                                echo '<input class="form-check-input modal-unit-checkbox" type="checkbox" name="assignments['.$unit->orgunit_id.'][selected]" value="1" data-id="'.$unit->orgunit_id.'" '.($isChecked ? 'checked' : '').'>';
                                                echo '<span class="form-check-label '.$isBold.'">'.$unit->name.'</span>';
                                                echo '</label>';
                                                echo '</div>';
                                                echo '</td>';
                                                echo '<td>';
                                                echo '<input type="text" class="form-control form-control-sm" name="assignments['.$unit->orgunit_id.'][target]" id="modal-target-'.$unit->orgunit_id.'" placeholder="Target..." value="'.$targetVal.'" '.$isDisabled.'>';
                                                echo '</td>';
                                                echo '</tr>';

                                                if ($unit->children && $unit->children->count()) {
                                                    foreach($unit->children as $child) {
                                                        renderUnitRowModalEdit($child, $level + 1, $assignedMap);
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    @foreach($orgUnits as $rootUnit)
                                        {{ renderUnitRowModalEdit($rootUnit, 0, $assignedMap) }}
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
...
@else
    ...
    @section('header')
    <x-tabler.page-header title="Edit Indikator" pretitle="SPMI / Indikator">
        <x-slot:actions>
