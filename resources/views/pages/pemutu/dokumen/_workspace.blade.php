...
$itemYear = $type === 'poin' ? ($item->dokumen->periode ?? session('siklus_spmi_tahun')) : ($item->periode ?? session('siklus_spmi_tahun'));
...
if ($jenis === 'standar') {
    $childrenColumns[] = ['data' => 'jumlah_turunan', 'title' => 'Jumlah Turunan'];
}

$childrenColumns[] = ['data' => 'action', 'title' => 'Aksi', 'orderable' => false, 'searchable' => false, 'class' => 'text-center'];

$columns = [
    'children' => $childrenColumns,
    'indikator' => [
        ['data' => 'no', 'name' => 'no', 'title' => '#', 'orderable' => false, 'searchable' => false, 'class' => 'text-center'],
    ...
    'mapping' => [
        ...
        ['data' => 'kode', 'title' => 'Kode'],
        ['data' => 'action', 'title' => 'Aksi', 'orderable' => false, 'searchable' => false, 'class' => 'text-center'],
    ],
    'poinChildren' => [
        ['data' => 'DT_RowIndex', 'title' => 'No', 'orderable' => false, 'searchable' => false, 'class' => 'text-center'],
...
<x-tabler.card >
    <x-tabler.card-body>
        <div class="d-flex justify-content-between align-items-start">
            ...
            <div class="d-flex gap-2 flex-shrink-0">
                <div class="btn-group">
                    @if($type === 'dokumen')
                        {{-- Approval Button --}}
                        @if($config->showApproval() && $canModify)
                            <x-tabler.button
                                type="success"
                    ...
                    @elseif($type === 'poin')
                        ...
                        @if(!$isKebijakan && $item->is_hasilkan_indikator && $canModify)
                            <x-tabler.button 
                                type="create" 
                                class="btn-success"
        ...
        <h2 class="mb-0">{{ $item->judul }}</h2>
        <ul class="nav nav-pills gap-2 my-2 border-bottom pb-2 mb-3" id="workspace-tabs-{{ $item->encrypted_doksub_id ?? ($item->encrypted_dok_id ?? 'root') }}" role="tablist">
        ...
        <div class="tab-content">
            ...
            @if($type === 'dokumen')
                ...
                @else
                    <div class="tab-pane" id="tab-subdokumen" role="tabpanel">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <h3 class="text-muted">Daftar {{ $childLabel ?? 'Turunan' }}</h3>
                            @if($canModify)
            ...
            <div class="tab-pane" id="tab-informasi" role="tabpanel">
                @if($type === 'dokumen')
                    <x-tabler.approval-history :approvals="$item->riwayatApprovals" :can-modify="$canModify" />
                @else
