    ...
    <div class="card-header">
        ...
        <div class="card-actions">
            <div class="btn-group" role="group">
                ...
                <a href="#" class="btn btn-outline-danger ajax-delete" data-url="{{ route('pemutu.org-units.destroy', $orgUnit->orgunit_id) }}" data-title="Delete Unit?" data-text="This will delete the unit and detach all pegawais.">
...
<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center w-100">
            ...
            {{-- Placeholder for Add Pegawai --}}
            <x-tabler.button class="btn-sm btn-outline-primary" disabled icon="ti ti-user-plus" text="Tambah Pegawai" />
        </div>
    </div>
    <div class="card-table">
        ...
        >
            @forelse($orgUnit->pegawai as $pegawai)
                <tr>
                    <td>
                        <div>{{ $pegawai->nama }}</div>
                        @if($pegawai->user)<div class="text-muted small">User Linked</div>@endif
                    </td>
                    <td>{{ $pegawai->email ?? '-' }}</td>
                    <td>{{ $pegawai->jenis ?? '-' }}</td>
                    <td>
        ...
        @if($orgUnit->pegawai->isEmpty())
            <div class="text-center text-muted p-3">No pegawai assigned.</div>
        @endif
    </div>
</div>
