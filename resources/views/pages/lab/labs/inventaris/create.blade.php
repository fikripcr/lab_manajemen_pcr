@if(request()->ajax() || request()->has('ajax'))
    <x-tabler.form-modal
        :title="'Tambah Inventaris ke Lab: ' . $lab->name"
        route="{{ route('lab.labs.inventaris.store', $lab->encrypted_lab_id) }}"
        method="POST"
        id="modal_inventaris_create"
    >
        <div class="mb-3">
            <x-tabler.form-select name="inventaris_id" id="inventaris_id_modal" label="Nama Alat" required style="width: 100%;">
            ...
            (function() {
                const initSelect2 = async () => {
                    if (typeof window.loadSelect2 === 'function') {
                        ...
                        $('#inventaris_id_modal').select2({
                            theme: 'bootstrap-5',
                            placeholder: 'Pilih atau ketik untuk mencari inventaris...',
                            allowClear: true,
                            width: '100%',
                            dropdownParent: $('#inventaris_id_modal').closest('.modal'),
                            ajax: {
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('lab.labs.inventaris.store', $lab->encrypted_lab_id) }}" method="POST" class="ajax-form">
                            ...
                            <div class="mb-3">
                                <x-tabler.form-select name="inventaris_id" id="inventaris_id" label="Nama Alat" required style="width: 100%;">
                                    @if(old('inventaris_id'))
                                        @php
                                            $selectedInventaris = \App\Models\Inventaris::find(decryptId(old('inventaris_id')));
                                        @endphp
