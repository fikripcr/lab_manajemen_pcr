@if(request()->ajax() || request()->has('ajax'))
    <div class="modal-header">
        <h5 class="modal-title">{{ $lab->name }}</h5>
    ...
    <div class="modal-body">
        ...
        <div class="mb-3">
            <h4 class="card-title mb-2">Informasi Laboratorium</h4>
        ...
        @if ($lab->getMedia('lab_images')->count() > 0)
            <div class="mb-3">
                <h4 class="card-title mb-2">Galeri</h4>
                <div class="row g-2">
                    @foreach ($lab->getMedia('lab_images')->take(4) as $media)
                        <div class="col-6">
                            <img src="{{ $media->getUrl() }}" class="img-fluid rounded border" alt="{{ $media->name }}" style="height: 100px; width: 100%; object-fit: cover;">
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="modal-footer">
        ...
        <x-tabler.button :href="route('lab.labs.show', $lab->encrypted_lab_id)" text="Detail Lengkap" />
    </div>
@else
