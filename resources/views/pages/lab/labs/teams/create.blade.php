@if(request()->ajax() || request()->has('ajax'))
    <x-tabler.form-modal
        :title="'Tambah Anggota Tim Lab: ' . $lab->name"
        route="{{ route('lab.labs.teams.store', $lab->encrypted_lab_id) }}"
        method="POST"
        id="modal_team_create"
    >
        <div class="mb-3">
            <x-tabler.form-select name="user_id" id="user_id_modal" label="Pilih User" required style="width: 100%;">
                @if(old('user_id'))
                    @php
            ...
            (function() {
                const initSelect2 = async () => {
                    if (typeof window.loadSelect2 === 'function') {
                        ...
                        $('#user_id_modal').select2({
                            theme: 'bootstrap-5',
                            placeholder: 'Pilih atau ketik untuk mencari user...',
                            allowClear: true,
                            width: '100%',
                            dropdownParent: $('#user_id_modal').closest('.modal'),
                            ajax: {
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('lab.labs.teams.store', $lab->encrypted_lab_id) }}" method="POST" class="ajax-form">
                            ...
                            <div class="mb-3">
                                <x-tabler.form-select name="user_id" id="user_id" label="Pilih User" required style="width: 100%;">
                                    @if(old('user_id'))
                                        @php
                                            $selectedUser = \App\Models\User::find(decryptId(old('user_id')));
                                        @endphp
