@if(request()->ajax() || request()->has('ajax'))
    <x-tabler.form-modal
        ...
        route="{{ route('lab.'.$type . '.update', $pengumuman->encrypted_pengumuman_id) }}"
        method="PUT"
        enctype="multipart/form-data"
        :submitText="'Update ' . ucfirst($type)"
    >
        <x-tabler.form-input name="judul" label="Title" value="{{ old('judul', $pengumuman->judul) }}" required />

        <x-tabler.form-textarea type="editor" id="isi_edit_modal" name="isi" label="Content" :value="old('isi', $pengumuman->isi)" height="400" required="true" />

        <div class="mb-3">
            <x-tabler.form-input type="file" id="cover_image_edit_modal" name="cover" label="Cover Image" accept="image/*" help="Upload a new cover image to replace the current one." />
        ...
        <div class="mb-3">
            <x-tabler.form-input type="file" id="attachments_edit_modal" name="attachments[]" label="Attachments" multiple="true" help="Upload additional files (current ones will be kept)." />
