        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-actions">
                    ...
                    <x-tabler.button type="button" icon="ti ti-plus" class="btn-sm ajax-modal-btn" data-url="{{ route('pemtu.label-types.create') }}" data-modal-title="Tambah Tipe Label" />
                </div>
            </div>
            <div class="list-group list-group-flush" id="label-type-list">
                <a href="#" class="list-group-item list-group-item-action active d-flex align-items-center" data-type-id="" onclick="filterLabels(this, '')">
                    All Labels
                    <span class="ms-auto badge bg-secondary-lt">{{ \App\Models\Pemtu\Label::count() }}</span>
                </a>
                @foreach($types as $type)
                    <div class="list-group-item list-group-item-action flex-column align-items-start label-type-item py-2" data-type-id="{{ $type->labeltype_id }}" onclick="filterLabels(this, '{{ $type->labeltype_id }}')">
                        <div class="d-flex w-100 justify-content-between align-items-center">
                            ...
                            <div class="btn-group">
                                 <button type="button" class="btn btn-sm btn-icon btn-ghost-secondary ajax-modal-btn" data-url="{{ route('pemtu.label-types.edit', $type->labeltype_id) }}" data-modal-title="Edit Tipe">
                                    <i class="ti ti-pencil"></i>
                                 </button>
                                 <button type="button" class="btn btn-sm btn-icon btn-ghost-danger ajax-delete" data-url="{{ route('pemtu.label-types.destroy', $type->labeltype_id) }}" data-title="Hapus Tipe?" data-text="Menghapus tipe akan menghapus semua label di dalamnya.">
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-actions">
                    <x-tabler.button type="button" icon="ti ti-plus" text="Tambah Label" class="btn-primary ajax-modal-btn" id="btn-add-label" data-url="{{ route('pemtu.labels.create') }}" data-modal-title="Tambah Label Baru" />
                </div>
            </div>
            <div class="card-body p-0">
                <x-tabler.datatable
                    id="table-labels"
                    route="{{ route('pemtu.labels.data') }}"
                    :columns="[
...
<script>
    ...
    function filterLabels(element, typeId) {
        ...
        const url = "{{ route('pemtu.labels.data') }}?type_id=" + typeId;
        table.ajax.url(url).load();

        // Update Add Label Button URL
        const addBtn = $('#btn-add-label');
        let addUrl = "{{ route('pemtu.labels.create') }}";
        if(typeId) {
